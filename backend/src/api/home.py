...
async def _with_cache(cache_key: str, ttl: int, compute):
    """Read-through cache: devuelve caché si existe, si no computa y cachea."""
    cached = await redis_service.get_cached_json(cache_key)
    if cached is not None:
        return cached
    result = await compute()
    await redis_service.set_cached_json(cache_key, result, ttl=ttl)
    return result
...
async def get_dashboard_stats(current_user: User = Depends(get_current_user)) -> Dict[str, Any]:
    ...
    return await _with_cache(f"cache:stats:{current_user.id}", ttl=30, compute=_compute)
...
async def get_recent_activity(current_user: User = Depends(get_current_user)) -> Dict[str, Any]:
    ...
    return await _with_cache(f"cache:recent:{current_user.id}", ttl=60, compute=_compute)
...
async def get_suggestions(current_user: User = Depends(get_current_user)) -> List[Dict[str, Any]]:
    ...
    return await _with_cache(f"cache:suggest:{current_user.id}", ttl=600, compute=_compute)
