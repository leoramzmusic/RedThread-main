...
async def check():
    ...
    print(f"SEARCH STATES: {len(p.search_states)}")
    for s in p.search_states:
        print(f"  STATE: {s.encode('ascii', 'ignore').decode()}")
    print(f"SEARCH COUNTRIES: {len(p.search_countries)}")
    for c in p.search_countries:
        print(f"  COUNTRY: {c.encode('ascii', 'ignore').decode()}")
